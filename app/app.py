...
with st.expander("What's real here vs. what's a placeholder"):
    st.markdown(
        ...
        "- **Real, tested against actual footage, mixed results**: uploading a delivery "
        ...
        "(`cv-pipeline/feature_extraction.py`). Run against 5 real WhatsApp clips: "
        "footwork detection looks genuinely useful (varied correctly across clips); "
        "**timing (`on_time`) did not** — every real clip read \"on time\", which turned "
        "out to be a structural flaw in how the heuristic is measured, not a good sign. "
        "See `cv-pipeline/README.md` for the honest breakdown.\n"
